...
#include <features/features_cpu.h>  // logging timestamps
...
static ssize_t alsa_write(void *data, const void *buf_, size_t size_)
{
	...
	snd_pcm_sframes_t avail, delay;  // for reporting buffer statistics
	...
	if (alsa->nonblock)
	{
		RARCH_DBG("[ALSA]: alsa_write() called (nonblocking)\n");
		while (size)
		{
			// Buffer statistics
			snd_pcm_avail_delay(alsa->pcm, &avail, &delay);
			RARCH_DBG("[ALSA]: %d frames available, %d frames delay @ T=%u\n",
					avail, delay, cpu_features_get_time_usec());
			...
			if (frames == -EPIPE || frames == -EINTR || frames == -ESTRPIPE)
			{
				...
				if (snd_pcm_recover(alsa->pcm, frames, 1) < 0) {
					RARCH_ERR("[ALSA]: snd_pcm_recover() error\n");
					return -1;
				}
				RARCH_DBG("[ALSA]: snd_pcm_recover() success\n");

				break;
			}
			else if (frames == -EAGAIN) {
				RARCH_ERR("[ALSA]: snd_pcm_writei() error %d \
						(EAGAIN)\n", frames);
				break;
			}
			else if (frames < 0) {
			...
			RARCH_DBG("[ALSA]: snd_pcm_writei() success\n");
			RARCH_DBG("[ALSA]: Wrote %d frames (%d total, %d remaining)\n",
					frames, written, size);
		}
	...
	{
		RARCH_DBG("[ALSA]: alsa_write() called (blocking)\n");
		...
		while (size)
		{
			...
			else if (frames == -EAGAIN)
			{
				...
				if (eagain_retry)
				{
					RARCH_DBG("[ALSA]: snd_pcm_writei() returns %d (EAGAIN), retrying\n", frames);
					eagain_retry = false;
					continue;
				}
				RARCH_ERR("[ALSA]: snd_pcm_writei() returns %d (EAGAIN), not retrying\n", frames);
				break;
			}
			else if (frames < 0) {
				RARCH_ERR("[ALSA]: snd_pcm_writei() error %d\n", frames);
